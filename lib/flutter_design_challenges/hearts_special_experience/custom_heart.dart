...
class CustomEnlightenedHeart extends StatefulWidget {
  const CustomEnlightenedHeart({
    Key? key,
    required this.musicAmplitude,
  }) : super(key: key);
  final double musicAmplitude;

  @override
  State<CustomEnlightenedHeart> createState() => _EnlightenedHeartState();
}

class _EnlightenedHeartState extends State<CustomEnlightenedHeart>
    ...
    return Scaffold(
      ...
      body: CustomPaint(
        painter: EnlightenedHeartPainter(
          musicAmplitude: widget.musicAmplitude,
...
class EnlightenedHeartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double heartRadius =
        max(size.width, size.height) * 0.015 + musicAmplitude * 2;
    ...
    for (var i = 0; i < 10; i++) {
      ...
      if (isInRange) {
        ...
        canvas.drawCircle(
          ...
          maxRadius * 0.35 + (shouldFlicker ? 0.7 : 0),
          circlesGroup3,
        );
      }
    }
    ...
    final circlesGroup4 = Paint()
      ..style = PaintingStyle.fill
      ..color = const Color.fromARGB(255, 255, 70, 206);
    for (var i = 0; i < 45; i++) {
